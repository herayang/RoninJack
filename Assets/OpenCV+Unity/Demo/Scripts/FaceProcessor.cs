namespace OpenCvSharp.Demo
{
        ...
        where T: UnityEngine.Texture
    {
        ...
        public float returnFaces()
        {
            // 1300 seems to be a good bet for the maximum x value aka XPosition should equal (x-650)/65 rounded
            if (Faces.Count != 0)
            {
                float x = Faces[0].Region.X + (Faces[0].Region.Width/2);
                return x;
            }
            else return 0;
        }
